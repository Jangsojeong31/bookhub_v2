...
| 이름 | 역할 | 담당 기능 |
|-----|-----|-----|
| 고혁재 | 백/프론트 | 로그인, JWT 토큰 인증, 권한 제어, 사원 관리(회원가입 승인, 사원 정보 변경, 정보 변경 로그 기록 등), 이메일 전송 기능 |
| 성재원 | 백/프론트 | 도서 관리(도서 CRUD, 도서 로그 기록), 수령 확인, 알림 기능, 메인화면 구성 |
| 장소정 | 백/프론트 | 작가 CRUD, 발주 및 발주 승인, 베스트셀러, 매출 통계, 리팩토링, 배포 |
| 최서윤 | 백/프론트 | 출판사 CRUD, 재고 관리(재고 변경, 재고 로그 기록), 매출 통계, 헤더 및 사이드바 구현 |
